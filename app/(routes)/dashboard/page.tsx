...
const Chatbotss = () => {
  ...
  const handleMessageSend = async () => {
    if (input.trim() !== "" || selectedImage) {
      // Check if input or image exists
  ...
  const sendMessage = async (text: string, image: File | null) => {
    ...
    try {
      const res = await axios.post<{ result: string }>(
        "https://4f5f-35-230-57-226.ngrok-free.app/generate",
        {
          query: text,
        }
      );
  ...
  return (
    <div
      className="min-h-screen flex text-white  bg-cover bg-center scrollbar-hidden"
      style={{
        backgroundImage: `url('../../public/96c2e8fda549ae99693e87ffeba899ef.jpg')`,
      }}
    >
      {/* Sidebar toggle button for small screens */}
      <button
        className="lg:hidden fixed top-4 left-4 z-5 p-2  text-white rounded-md"
      ...
      {/* This is sidebar */}
      <div
        className={`fixed w-64 h-screen   overflow-y-auto z-40 transform ${
      ...
      >
        <div className="p-5 h-full flex flex-col">
          <div className="flex mb-15">
            ...
            <span
              className="text-4xl left-1/2 font-medium  mt-2"
              style={{ color: "#4E4E4E" }}
            >
              .ExeDeveloper
            </span>
          ...
          <motion.button
            ...
            className="w-full py-4 mb-5 mt-5 rounded-md relative bg-white text-black"
          >
          ...
          <motion.button
            ...
            className="w-full py-4 mb-10 rounded-md relative unique-radial-gradient"
          >
          ...
          <div className="space-y-5 flex-grow overflow-auto pr-4 mb-24">
            {chatSessions.map((session) => (
              <div
                key={session.id}
                className={`relative p-3 rounded-lg border border-black text-black text-sm hover:shadow-[4px_4px_0px_0px_rgba(0,0,0)] transition duration-200 cursor-pointer ${
                  session.id === activeSessionId
                    ? "bg-[#FFE0BD] font-medium shadow-[4px_4px_0px_0px_rgba(0,0,0)]"
                    : "hover:bg-[#FFE0BD]"
                }`}
      ...
      <div
        className={`flex-1  flex flex-col transition-transform duration-300 ease-in-out ${
          isSidebarOpen ? "ml-0 blur-md opacity-50 pointer-events-none" : "ml-0"
        } lg:ml-64 mb-12`}
      >
        <div className="flex-1  overflow-x-hidden  p-4 ">
          {activeSession ? (
            <>
                  ...
                  <div
                    className={`inline-flex ${
                      message.isBot
                        ...
                        : "bg-white rounded-t-2xl rounded-bl-2xl"
                    } shadow-lg backdrop-blur-md p-3 max-w-[75%]`}
                    style={{
              ...
              {isBotTyping && (
                <div className="p-2 max-w-[5rem] bg-[#f1b263] rounded-lg">
                  <p>...</p>
                </div>
              )}
            </>
        ...
        <div className="fixed z-50 bottom-0 left-1/2 transform -translate-x-1/2 w-full flex justify-center px-4">
          ...
          <div>
            ...
            <form className="rounded-md">
              ...
              <div className="relative w-full grow flex flex-col overflow-hidden bg-background dark:bg-bgDarkSecondary md:rounded-md border rounded-lg blackbox-textarea-hover-color mb-4">
                  ...
                  <div className="absolute right-2 top-4">
                    {!isSidebarOpen && ( // Conditionally render the send button
                      <button
                        ...
                        className={`bg-transparent border-none flex items-center justify-center p-2 ${
                      ...
                      >
                        <FaPaperPlane
                          className={`text-lg ${
                            input.trim() ? " text-[#FFA947] " : "text-gray-600"
                          }`}
                        />
                      </button>
                    )}
